...
// hereOK();
	...
	if (recPos!=0L && writeNames)
	{
		while (1)
		{
			...
			if (tempRecord.ensemblConsequence[0]!='\0')
				sprintf(buff,"%ld-%s",tempRecord.pos,tempRecord.ensemblConsequence);
			else if (tempRecord.quickConsequence[0]!='\0')
				sprintf(buff,"%ld-%s",tempRecord.pos,tempRecord.quickConsequence);
			else
				sprintf(buff,"%ld",tempRecord.pos);
			buff[19]='\0';
			sscanf(buff,"%s",buff2); // in case there are two words in it
			fprintf(fp,"%s ",buff2);
			recPos=index.get_next();
			if (recPos==0L)
				break;
	...
	if (writeComments)
		recPos=findFirstInRange(spec);
	if (recPos!=0L && writeComments)
		// I know it is a bit silly to be going through this iteration again - maybe I will fix it some day 
	{
		while (1)
		{
			testKey=index.current_key();
